...
export default function AddShiftScreen({ navigation, route }) {
  ...
  //           style={styles.select}
  //         >
  //           <option value="" disabled style={styles.input}>Select {label.toLowerCase()}</option>
  //           {list.map((item) => (
  //             <option key={item} value={item} style={styles.inputLabel}>{item}</option>
  //           ))}
  //         </select>
  //       </View>
  //     );
  ...
  const renderDropdown = (value, setValue, showDropDown, setShowDropDown, list, label) => {
    if (Platform.OS === 'web') {
      return (
        <View style={styles.selectContainer}>
          <Title style={[styles.inputLabel, { textAlign: 'left' }]}>{label}*</Title>
          <select
            value={value}
            onChange={(e) => setValue(e.target.value)}
            style={[styles.select, { textAlign: 'left', textAlignLast: 'left' }]}
          >
            <option value="" disabled>Select {label.toLowerCase()}</option>
            {list.map((item) => (
              <option key={item} value={item} style={[styles.inputLabel, { textAlign: 'left' }]}>{item}</option>
            ))}
          </select>
        </View>
      );
      ...
      return (
        <Menu
          ...
          onDismiss={() => setShowDropDown(false)}
          anchor={
            <Button mode="outlined" onPress={() => setShowDropDown(true)} style={[styles.input, { textAlign: 'left' }]}>
              {value || `Select ${label}*`}
            </Button>
          }
        >
  ...
  return (
    <View style={styles.container}>
      <TextInput
        ...
        labelStyle={[styles.label, { textAlign: 'left' }]}
        mode="outlined"
      />
      {renderDropdown(
        activity,
...
const styles = StyleSheet.create({
  ...
  select: {
    ...
    paddingLeft: 10,
    textAlign: 'left', // Align the text to the left
    textAlignLast: 'left', // Align the last option to the left
  ...
  inputLabel: {
    fontSize: 12,
    color: '#666',
    textAlign: 'left', // Align the input label text to the left
    width: '100%',
  },
  label: {
    fontSize: 14,
